...
void main(string[] input){
	...
	//text image
	enum textsize=960-margin*2;
	executeShell("rm temp.png");
	executeShell(
		"convert -size "~textsize.to!string~"x -font Helvetica -pointsize 32 caption:@"
		~textfile~" -fill black temp.png"
		);
	ubyte[] text=cast(ubyte[])executeShell("convert temp.png rgb:-").output;
	ubyte[textsize*3] blankline=255;
	ulong textlength=text.length/(textsize*3);
	...
	foreach(frame;0..mp3time){
		foreach(y;0..1080){
			...
			y-=offset;
			write(blankline[0..margin*3]);
			if(y<0 || y>=textlength){
				write(blankline[]);
			} else {
				write(text[(textsize*3)*y..(textsize*3)*(y+1)]);
			}
			write(blankline[0..margin*3]);
	}}
}